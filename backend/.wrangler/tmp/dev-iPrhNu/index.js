...
// .wrangler/tmp/bundle-CMOCso/checked-fetch.js
...
// .wrangler/tmp/bundle-CMOCso/strip-cf-connecting-ip-header.js
...
var t = /* @__PURE__ */ __name(({ base: e = "", routes: t2 = [], ...r2 } = {}) => ({ __proto__: new Proxy({}, { get: (r3, o2, a2, s2) => (r4, ...c) => t2.push([o2.toUpperCase?.(), RegExp(`^${(s2 = (e + r4).replace(/\/+(\/|$)/g, "$1")).replace(/(\/?\.?):(\w+)\+/g, "($1(?<$2>*))").replace(/(\/?\.?):(\w+)/g, "($1(?<$2>[^$1/]+?))").replace(/\./g, "\\.").replace(/(\/?)\*/g, "($1.*)?")}/*$`), c, s2]) && a2 }), routes: t2, ...r2, async fetch(e2, ...o2) {
...
var generateCode = /* @__PURE__ */ __name(() => Math.floor(1e5 + Math.random() * 9e5).toString(), "generateCode");
var sendVerificationEmail = /* @__PURE__ */ __name(async (email, code, env) => {
  const emailData = {
    from: "DrawDB <onboarding@resend.dev>",
    to: [email],
    subject: "Your DrawDB Verification Code",
    html: `Your verification code is: <strong>${code}</strong>. It will expire in 10 minutes.`
  };
...
router.post("/api/auth/login", async (request, env) => {
  ...
  if (!email) {
    return s(400, "Email is required.");
  }
  ...
  try {
    ...
    return o({ success: true, message: "Verification code sent." });
  } catch (e) {
    console.error(e);
    return s(500, "Failed to send verification code.");
  }
});
router.post("/api/auth/verify", async (request, env) => {
  const { email, code } = await request.json();
  if (!email || !code) {
    return s(400, "Email and code are required.");
  }
  const { results } = await env.DB.prepare(
    "SELECT * FROM auth_codes WHERE email = ? AND code = ?"
  ).bind(email, code).all();
  const authCode = results[0];
  if (!authCode || /* @__PURE__ */ new Date() > new Date(authCode.expires_at)) {
    return s(401, "Invalid or expired code.");
  }
  ...
  const secret = new TextEncoder().encode(env.JWT_SECRET);
  const token = await new SignJWT({ userId, email }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("7d").sign(secret);
  return o({ token });
});
router.get("/api/diagrams", authMiddleware, async (request, env) => {
...
router.post("/api/diagrams", authMiddleware, async (request, env) => {
  const userId = request.userId;
  const { name, content } = await request.json();
  const diagramId = v4_default();
  await env.DB.prepare(
    "INSERT INTO diagrams (id, name, content, owner_id) VALUES (?, ?, ?, ?)"
  ).bind(diagramId, name || "Untitled Diagram", JSON.stringify(content), userId).run();
  return o({ id: diagramId });
});
router.get("/api/diagrams/:id", authMiddleware, async (request, env) => {
  const userId = request.userId;
  ...
  if (!diagram) {
    return s(404, "Diagram not found or access denied.");
  }
  return o(diagram);
});
router.put("/api/diagrams/:id", authMiddleware, async (request, env) => {
  const userId = request.userId;
  const { id } = request.params;
  const { name, content } = await request.json();
  const diagram = await env.DB.prepare(
  ...
  ).bind(name, JSON.stringify(content), id).run();
  return o({ success: true });
});
router.post("/api/diagrams/:id/share", authMiddleware, async (request, env) => {
  const ownerId = request.userId;
  const { id: diagramId } = request.params;
  const { email: collaboratorEmail } = await request.json();
  const diagram = await env.DB.prepare("SELECT id FROM diagrams WHERE id = ? AND owner_id = ?").bind(diagramId, ownerId).first();
  if (!diagram) {
    return s(403, "Only the owner can share this diagram.");
  }
  const collaborator = await env.DB.prepare("SELECT id FROM users WHERE email = ?").bind(collaboratorEmail).first();
  if (!collaborator) {
    return s(404, "Collaborator user not found.");
  }
  await env.DB.prepare(
    "INSERT OR IGNORE INTO diagram_collaborators (diagram_id, user_id) VALUES (?, ?)"
  ).bind(diagramId, collaborator.id).run();
  return o({ success: true, message: "Diagram shared successfully." });
});
router.delete("/api/diagrams/:id", authMiddleware, async (request, env) => {
  const userId = request.userId;
  const { id } = request.params;
  const diagram = await env.DB.prepare("SELECT id FROM diagrams WHERE id = ? AND owner_id = ?").bind(id, userId).first();
  if (!diagram) {
    return s(403, "Only the owner can delete this diagram.");
  }
...
// .wrangler/tmp/bundle-CMOCso/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CMOCso/middleware-loader.entry.ts
